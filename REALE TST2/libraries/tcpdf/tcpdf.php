...
class TCPDF {
		...
		if (is_string($format)) {
			...
			if (isset($format['trans'])) {
				...
				if (isset($format['trans']['S']) && in_array($format['trans']['S'], $stansition_styles)) {
					...
					if (isset($format['trans']['Di']) && in_array($format['trans']['S'], $valid_effect)) {
						if (((($format['trans']['Di'] == 90) || ($format['trans']['Di'] == 180)) && ($format['trans']['S'] == 'Wipe'))
							|| (($format['trans']['Di'] == 315) && ($format['trans']['S'] == 'Glitter'))
							|| (($format['trans']['Di'] == 0) || ($format['trans']['Di'] == 270))) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (TCPDF_STATIC::empty_string($fontfile) || (!@file_exists($fontfile))) {
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode) {
			if ((($opt['Subtype'] == 'FileAttachment') || ($opt['Subtype'] == 'Sound')) && (!TCPDF_STATIC::empty_string($opt['FS']))
				&& (@file_exists($opt['FS']) || TCPDF_STATIC::isValidURL($opt['FS']))
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				if (($l > $wmax) || (($c == 173) && (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl && ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							|| ((!$this->rtl) && ($this->x >= ($this->lMargin + $this->cell_padding['L'] + $margin['L'] + $chrwidth)))) {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		} elseif (($ismask === false) && ($imgmask === false) && (strpos($file, '__tcpdf_imgmask_') === FALSE)) {
		...
		if ($newimage) {
			...
			if ((method_exists('TCPDF_IMAGES', $mtd)) && (!($resize && (function_exists($gdfunction) || extension_loaded('imagick'))))) {
				...
				if (($ismask === false) && ($imgmask === false) && (strpos($file, '__tcpdf_imgmask_') === FALSE)
					&& (($info === 'pngalpha') || (isset($info['trns']) && !empty($info['trns'])))) {
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		if ($destroyall && isset($this->diskcache) && $this->diskcache && (!$preserve_objcopy) && (!TCPDF_STATIC::empty_string($this->buffer))) {
			// remove buffer file from cache
			unlink($this->buffer);
		}
		if ($destroyall && !empty($this->cached_files)) {
			// remove cached files
		...
		foreach (array_keys(get_object_vars($this)) as $val) {
			if ($destroyall || !in_array($val, $preserve)) {
	...
	protected function _putcatalog() {
		...
		if (!empty($this->form_obj_id)
			...
			// signatures
			if ($this->sign && isset($this->signature_data['cert_type']) 
				&& (empty($this->signature_data['approval']) || ($this->signature_data['approval'] != 'A'))) {
				if ($this->signature_data['cert_type'] > 0) {
	...
	public function addHtmlLink($url, $name, $fill=false, $firstline=false, $color='', $style=-1, $firstblock=false) {
		if (isset($url[1]) && ($url[0] == '#')) {
			...
			if (isset($lnkdata[0]) ) {
				...
				if (isset($lnkdata[1]) && (strlen($lnkdata[1]) > 0)) {
	 ...
	 * @param $tsa_username (string) Specifies the username for TSA authorization (optional) || specifies the TSA authorization PEM file (see: example_66.php, optional)
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		...
		if ($style['stretch']) {
			...
			if ((isset($style['padding']) && ($style['padding'] === 'auto'))
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if ((($dom[$key]['value'] == 'ul') || ($dom[$key]['value'] == 'ol') || ($dom[$key]['value'] == 'dl'))
						&& (!isset($dom[$key]['align']) || TCPDF_STATIC::empty_string($dom[$key]['align']) || ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] && isset($dom[$key]['attribute']['pagebreak'])) {
				...
				if ((($dom[$key]['attribute']['pagebreak'] == 'left') && (((!$this->rtl) && (($this->page % 2) == 0)) || (($this->rtl) && (($this->page % 2) != 0))))
					|| (($dom[$key]['attribute']['pagebreak'] == 'right') && (((!$this->rtl) && (($this->page % 2) != 0)) || (($this->rtl) && (($this->page % 2) == 0))))) {
					// add a page (or trig AcceptPageBreak() for multicolumn mode)
					$this->checkPageBreak($this->PageBreakTrigger + 1);
					$this->htmlvspace = ($this->PageBreakTrigger + 1);
				}
			...
			if (($dom[$key]['value'] == 'tr') && isset($dom[$key]['thead']) && $dom[$key]['thead']) {
				...
				while ( ($key < $maxel) && (!(
					($dom[$key]['tag'] && $dom[$key]['opening'] && ($dom[$key]['value'] == 'tr') && (!isset($dom[$key]['thead']) || !$dom[$key]['thead']))
					|| ($dom[$key]['tag'] && (!$dom[$key]['opening']) && ($dom[$key]['value'] == 'table'))) )) {
					++$key;
				}
			}
				...
				if ((!$this->newline) && ($dom[$key]['value'] == 'img') && (isset($dom[$key]['height'])) && ($dom[$key]['height'] > 0)) {
					...
					if (!empty($dom[$key]['width'])) {
						...
						if (($imgw <= ($this->w - $this->lMargin - $this->rMargin - $this->cell_padding['L'] - $this->cell_padding['R']))
							&& ((($this->rtl) && (($this->x - $imgw) < ($this->lMargin + $this->cell_padding['L'])))
							|| ((!$this->rtl) && (($this->x + $imgw) > ($this->w - $this->rMargin - $this->cell_padding['R']))))) {
					...
					if (($fontname != $curfontname) || ($fontstyle != $curfontstyle) || ($fontsize != $curfontsize)
						...
						|| ($dom[$key]['tag'] && $dom[$key]['opening'] && ($dom[$key]['value'] == 'li')) ) {
						if (($key < ($maxel - 1)) && (
								($dom[$key]['tag'] && $dom[$key]['opening'] && ($dom[$key]['value'] == 'li'))
								|| ($this->cell_height_ratio != $dom[$key]['line-height'])
								|| (!$this->newline && is_numeric($fontsize) && is_numeric($curfontsize) && ($fontsize >= 0) && ($curfontsize >= 0) && ($fontsize != $curfontsize))
							)) {
							if ($this->page > $startlinepage) {
			...
			if ($this->newline && (strlen($dom[$key]['value']) > 0) && ($dom[$key]['value'] != 'td') && ($dom[$key]['value'] != 'th')) {
				...
				if (!($dom[$key]['tag'] && !$dom[$key]['opening'] && ($dom[$key]['value'] == 'table')
					&& (isset($this->emptypagemrk[$this->page]))
					&& ($this->emptypagemrk[$this->page] == $this->pagelen[$this->page]))) {
					$this->SetFont($fontname, $fontstyle, $fontsize);
					if ($wfill) {
					...
					if (($dom[$key]['value'] == 'td') || ($dom[$key]['value'] == 'th')) {
						...
						if (isset($dom[$table_el]['rowspans'])) {
							...
							while ($rsk < $rskmax) {
								...
								if (($trwsp['rowspan'] > 0)
									&& ($rsstartx > ($this->x - $cellspacing['H'] - $current_cell_padding['L'] - $this->feps))
									&& ($rsstartx < ($this->x + $cellspacing['H'] + $current_cell_padding['R'] + $this->feps))
									&& (($trwsp['starty'] < ($this->y - $this->feps)) || ($trwsp['startpage'] < $this->page) || ($trwsp['startcolumn'] < $this->current_column))) {
									...
									if (($trwsp['rowspan'] == 1)
										&& (isset($dom[$trid]['endy']))
										&& (isset($dom[$trid]['endpage']))
										&& (isset($dom[$trid]['endcolumn']))
										&& ($trwsp['endpage'] == $dom[$trid]['endpage'])
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		if ($dom[$key]['self'] && isset($dom[$key]['attribute']['pagebreakafter'])) {
			...
			if ((($pba == 'left') && (((!$this->rtl) && (($this->page % 2) == 0)) || (($this->rtl) && (($this->page % 2) != 0))))
				|| (($pba == 'right') && (((!$this->rtl) && (($this->page % 2) != 0)) || (($this->rtl) && (($this->page % 2) == 0))))) {
				// add a page (or trig AcceptPageBreak() for multicolumn mode)
				$this->checkPageBreak($this->PageBreakTrigger + 1);
			}
		}
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		...
		switch($tag['value']) {
			case 'tr': {
				...
				if ($this->num_columns > 1) {
					if ((($this->current_column == 0) && ($dom[($dom[$key]['parent'])]['endcolumn'] == ($this->num_columns - 1)))
						|| (($this->current_column == $dom[($dom[$key]['parent'])]['endcolumn']) && ($prev_page < $this->page))) {
						// page jump
						$this->selectColumn(0);
	...
	protected function setBuffer($data) {
		...
		if ($this->diskcache) {
			if (!isset($this->buffer) || TCPDF_STATIC::empty_string($this->buffer)) {
	...
	*/
	...
	public function addPageRegion($region) {
		...
		if (isset($region['xt']) && isset($region['xb']) && ($region['xt'] > 0) && ($region['xb'] > 0)
			&& isset($region['yt'])  && isset($region['yb']) && ($region['yt'] >= 0) && ($region['yt'] < $region['yb'])
			&& isset($region['side']) && (($region['side'] == 'L') || ($region['side'] == 'R'))) {
	...
	protected function checkPageRegions($h, $x, $y) {
		...
		foreach ($this->page_regions as $regid => $regdata) {
			if ($regdata['page'] == $this->page) {
				...
				if (($y > ($regdata['yt'] - $h)) && ($y <= $regdata['yb'])) {
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		switch($name) {
			...
			case 'linearGradient': {
				...
				if (((!isset($attribs['x1'])) && (!isset($attribs['y1'])) && (!isset($attribs['x2'])) && (!isset($attribs['y2'])))
					|| ((isset($attribs['x1']) && (substr($attribs['x1'], -1) == '%'))
						|| (isset($attribs['y1']) && (substr($attribs['y1'], -1) == '%'))
						|| (isset($attribs['x2']) && (substr($attribs['x2'], -1) == '%'))
						|| (isset($attribs['y2']) && (substr($attribs['y2'], -1) == '%')))) {
			...
			case 'radialGradient': {
				...
				//$attribs['spreadMethod']
				if (((!isset($attribs['cx'])) && (!isset($attribs['cy'])))
					|| ((isset($attribs['cx']) && (substr($attribs['cx'], -1) == '%'))
						|| (isset($attribs['cy']) && (substr($attribs['cy'], -1) == '%')) )) {
					$this->svggradients[$this->svggradientid]['mode'] = 'percentage';
				} else {
					$this->svggradients[$this->svggradientid]['mode'] = 'measure';
				}
