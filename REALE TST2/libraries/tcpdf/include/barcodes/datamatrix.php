...
// -----------------------------------------------------------------------------
	...
	protected function isCharMode($chr, $mode) {
		...
		switch ($mode) {
			...
			case ENC_BASE256: { // Function character (FNC1, Structured Append, Reader Program, || Code Page)
		...
		while (true) {
			...
			if ($charscount >= 4) {
				...
				if ((($numch[ENC_BASE256] + 1) <= $numch[ENC_ASCII])
					|| (($numch[ENC_BASE256] + 1) < min($numch[ENC_C40], $numch[ENC_TXT], $numch[ENC_X12], $numch[ENC_EDF]))) {
					return ENC_BASE256;
				}
		...
		while ($pos < $data_lenght) {
			...
			switch ($enc) {
				case ENC_ASCII: { // STEP B. While in ASCII encodation
					if (($data_lenght > 1) && ($pos < ($data_lenght - 1)) && ($this->isCharMode(ord($data[$pos]), ENC_ASCII_NUM) && $this->isCharMode(ord($data[$pos + 1]), ENC_ASCII_NUM))) {
				...
				case ENC_EDF: { // F. While in EDIFACT (EDF) encodation
					...
					do {
						...
						if (($field_lenght == 4) || ($epos == $data_lenght) || !$this->isCharMode($chr, ENC_EDF)) {
							if (($epos == $data_lenght) && ($field_lenght < 3)) {
								$enc = ENC_ASCII;
								$cw[] = $this->getSwitchEncodingCodeword($enc);
								++$cw_num;
								break;
				...
				case ENC_BASE256: { // G. While in Base 256 (B256) encodation
					...
					$temp_cw = array();
					$field_lenght = 0;
					while (($pos < $data_lenght) && ($field_lenght <= 1555)) {
						$newenc = $this->lookAheadTest($data, $pos, $enc);
						if ($newenc != $enc) {
