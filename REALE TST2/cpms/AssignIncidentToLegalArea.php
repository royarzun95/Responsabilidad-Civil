...
/**
* CPMObjectEventHandler: AssignIncidentToLegalArea
* Package: RN
* Objects: Incident
* Actions: Update
* Version: 1.3
...
*/
class AssignIncidentToLegalArea implements RNCPM\ObjectEventHandler
{
  /**
  ...
  */
  public static function apply($runMode, $action, $incident, $cycle)
  {
    ...
    try
    {
      $ids     = RNCPHP\Configuration::fetch('CUSTOM_CFG_LEGAL_ID_RC')->Value;
      if(isset($ids))
      {      
        $chooser = explode(",", $ids); 
        $random  = array_rand($chooser,1);
        $persona = RNCPHP\Account::fetch($chooser[$random]);
        $incident->AssignedTo->Account = $persona;
        $incident->save();
        self::insertPrivateNote($incident, "Ticket enviado al Area legal y asignado a ".$persona->Name->First." ".$persona->Name->Last.".");
      }
      else
      {
        self::insertPrivateNote($incident, "No se ha encontrado valor en el valor de configuracion.");
        return;
      }
...
class AssignIncidentToLegalArea_TestHarness implements RNCPM\ObjectEventHandler_TestHarness
{
